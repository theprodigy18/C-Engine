...
bool Graphics_CreateGraphics(GfxHandle* pHandle, const GfxInitProps* pInitProps)
{
    if (!s_isInitialized)
    {
        WndInitProps dummyWIP;
        dummyWIP.title  = "Dummy";
        dummyWIP.width  = 800;
        dummyWIP.height = 600;

        WndHandle dummyHandle = NULL;
        if (!Platform_CreateWindow(&dummyHandle, &dummyWIP))
        {
            ASSERT(false);
            return false;
        }
        ...
        i32 pixelFormat = ChoosePixelFormat(dummyHandle->hdc, &pfd);
        if (!pixelFormat)
        {
            ASSERT(false);
            return false;
        ...
        if (!SetPixelFormat(dummyHandle->hdc, pixelFormat, &pfd))
        {
            ASSERT(false);
            return false;
        }

        HGLRC dummyContext = wglCreateContext(dummyHandle->hdc);
        if (!dummyContext)
        {
            ASSERT(false);
            return false;
        ...
        if (!wglMakeCurrent(dummyHandle->hdc, dummyContext))
        {
            ASSERT(false);
            return false;
        }
